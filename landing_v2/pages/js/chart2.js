...
function zoomed2(zoomType) {
        console.log(typeof(zoomType))
...
//            console.log(e.translate[0])
            ...
            // zoom was initiated by a the scroll bar
            console.log("scrool zoom:"+zoomType)
            
            e = new Event("MouseEvents"); // overwrite e with a new d3-like object
            e.scale = zoomType; // change the target scale

             // translation math
            tx = Math.min(0, Math.max(1, visWidth - visWidth * zoomType));
            ty = Math.min(0, Math.max(1, visHeight - visHeight * zoomType));
...
var topBuffer = svgContainer.append( "rect" )
    ...
    .attr("height",10)
    .attr("width",visWidth)
    .attr("fill", "white") // Beige
//    .attr("stroke","red")
//    .attr("stroke-width","1")
//    .style('opacity', 0.95);

var topLine = svgContainer.append( "line" )
    .attr("x1", 0)            
    .attr("y1", 10)
    .attr("x2", visWidth)            
    .attr("y2", 10)
