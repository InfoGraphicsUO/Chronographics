...
//      
	...
	powersGroup.selectAll("path").data(powerVar).enter().append( "path" )
		...
		.attr( "fill", function(d){return lookupGradient(d.properties.color);} ) // fill with watercolor image
...
function clearSelection(bool){
	console.log("clearing - functions2")
	// click selection labels
	d3.select("#place_label").text("")
	d3.select("#power_label").text("")
	d3.select("#year_label").text("")
	d3.select("#power_map").text("click map or chart to select")
	...
	d3.select("#year_label").style("font-style: italic;")
	d3.select("#graph_label").text("Make a selection on the chart to show the graphs.")
	//d3.select("#descriptive_text").text("Click map or chart to view Priestley's place description.");
...
//function sizeChange(currentZoom){
//	// Reset Chart
//	zoomChart.translate([0, 0]).scale(currentZoom);
//	svgContainer.attr("transform", [
//		"translate(" + 0 + "," + 0 + ")",
//		"scale(" + currentZoom + ")"
//	].join(" "));
